...
mod tests {
    ...
    fn sanitizes_invalid_selector_chars() {
        ...
        let out = normalize_generated_css(input);
        assert!(out.contains(".foo\\(bar\\)"), "parentheses should be escaped: {out}");
    ...
    fn reescapes_leading_invalid_identifier_chars() {
        ...
        let out = normalize_generated_css(input);
        assert!(out.contains(".\\*loading\\(bg-gray-400\\)"), "Expected leading '*' to be escaped: {out}");
    }
    ...
    #[test]
    fn does_not_escape_decimal_literals_in_declarations() {
        ...
        assert!(!out.contains("opacity:\\.75"), "Should not escape decimal point in declaration: {out}");
    }
    ...
    #[test]
    fn preserves_and_escapes_parametric_symbol_group() {
        let input = ".\\~text\\(2.25rem\\@md,\\ 3rem\\@xl\\){font-size:clamp(2.25rem, calc(2.25rem + 0.75 * (100vw - 768px) / (1280 - 768)), 3rem);}";
        let out = normalize_generated_css(input);
        let expected_selector = ".\\~text\\(2\\.25rem\\@md\\,\\ 3rem\\@xl\\)";
        assert!(out.contains(expected_selector), "Expected preserved and fully sanitized parametric symbol group: {out}");
        assert!(StyleSheet::parse(&out, ParserOptions::default()).is_ok(), "fully sanitized CSS failed to parse: {out}");
    }
    ...
    fn fixes_missing_dot_for_escaped_symbol_group() {
        ...
        let out = normalize_generated_css(input);
        assert!(out.starts_with("."), "Should insert missing dot for escaped symbol group: {out}");
        assert!(StyleSheet::parse(&out, ParserOptions::default()).is_ok(), "CSS with fixed dot failed to parse: {out}");
    }
}
